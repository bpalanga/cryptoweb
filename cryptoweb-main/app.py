...
def vault():
    ...
    if session['role'] == 'admin':
        # ADMIN can view ALL cards (full decryption happens in the query)
        cursor.execute("""
        ...
        for i in range(len(cards)):
            ...
            # Mask the decrypted data before display (standard practice)
            cards[i][2] = mask_card_number(cards[i][2]) 
            cards[i][3] = mask_cvv(cards[i][3])
            cards[i] = tuple(cards[i])

    elif session['role'] in ['merchant', 'customer']: # <--- CHANGED ROLES
        # Merchant and Customer can only view their OWN cards (encrypted data retrieved)
        # Assuming only the user's own cards should be visible to Merchant/Customer for this level of access
        
        # NOTE: This query will need adjustment. The existing code retrieves ALL cards, which is likely wrong for non-admin roles.
        # It should probably filter by the current user's ID.
        
        cursor.execute("""
        SELECT id, userid, card_number, CVV, name, billing_address
        FROM CardDetails
        WHERE userid = %s  -- <--- ADDED FILTER to ensure non-admins only see their own data
        """, (session['userid'],))
        
        cards = cursor.fetchall()
        for i in range(len(cards)):
            cards[i] = list(cards[i])
            # Mask the raw encrypted data before display
            cards[i][2] = mask_card_number(cards[i][2])
            cards[i][3] = mask_cvv(cards[i][3])
            cards[i] = tuple(cards[i])
...
def decrypt_card(card_id):
    if 'userid' not in session or session['role'] not in ['admin', 'merchant', 'customer']:
        return redirect('/')
